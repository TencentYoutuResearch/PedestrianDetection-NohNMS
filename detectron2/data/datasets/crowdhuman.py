...
def load_crowdhuman(anno_file, image_dir):
    ...
    for img_id, anno in enumerate(annos):
        ...
        for gt_box in anno["gtboxes"]:
            ...
            vis_ratio = (gt_box["vbox"][2] * gt_box["vbox"][3]) / float((gt_box["fbox"][2] * gt_box["fbox"][3]))
    ...
    logger.info("Loaded {} instances and {} ignore instances in CrowdHuman from {}".format(instances, ignore_instances, anno_file))
    ...
    for img_id, anno in tqdm(enumerate(annos)):
        ...
        for gt_box in anno["gtboxes"]:
            ...
            if gt_box["tag"] != "person" or gt_box["extra"].get("ignore", 0) == 1:
                ...
                annotation["ignore"] = 1
                outside_num += 1
            elif is_clip and ((bbox[0] < 0) or (bbox[1] < 0) or (bbox[2] > width) or (bbox[3] > height)):
